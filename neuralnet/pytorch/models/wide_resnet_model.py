...
class wide_basic(nn.Module):
  def __init__(self, in_planes, planes, dropout, stride=1, leaky_slope=0.0,
               bias=True):
  ...
  def forward(self, x):
    out = self.dropout(self.conv1(self.leaky_relu(self.bn1(x))))
...
class WideResnetModel(nn.Module):
  ...
  def __init__(self, params, num_classes, is_training):
    ...
    self.bias = config.get('bias', None) or True
  ...
  def _wide_layer(self, block, planes, num_blocks, stride):
    ...
    for stride in strides:
      layers.append(block(
        self.in_planes, planes, self.dropout, stride,
        leaky_slope=self.leaky_slope, bias=self.bias))
